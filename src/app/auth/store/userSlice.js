...
export const setUserDataFirebase =
    (user, authUser) => async (dispatch, getState) => {
        const { permissions } = getState().auth.user;
        if (
            user &&
            user.data &&
            user.data.settings &&
            ...
            return dispatch(
                setUserData({ ...user, permissions: [...permissions] })
            );
        }
...
export const createUserSettingsFirebase =
    (authUser) => async (dispatch, getState) => {
        ...
        const user = _.merge({}, guestUser, {
            uid: authUser.uid,
            from: "firebase",
            businessTypeId: authUser.typeId,
            verified: false,
            data: {
...
export const { setUser, userLoggedOut, updateTemplate } = userSlice.actions;
